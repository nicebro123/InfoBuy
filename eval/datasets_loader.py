...
class DatasetHandler(ABC):
    ...
    def extract_answer(self, response: str) -> str:
        if self.answer_pattern == ANSWER_PATTERN_BOXED:
            return extract_last_boxed_content(response)
        matches = list(re.finditer(self.answer_pattern, response))
    ...
    def compare_answer(self, response: str, answer: str) -> bool:
        ...
        return verify(parse(answer), parse(str(response_answer)))
...
class LocalJsonDatasetHandler(DatasetHandler):
    def __init__(self, path: str, answer_pattern: str = ANSWER_PATTERN_BOXED):
        super().__init__(answer_pattern)
        if not path:
            raise ValueError("Dataset local_json requires --name /path/to/train.jsonl.")
        self.path = Path(path)
    ...
    def load_data(self):
        ...
        if not isinstance(examples, list):
            raise ValueError(f"{self.path} must contain a JSON list or JSONL records.")

        questions = []
        answers = []
        for index, example in enumerate(examples):
            question = example.get("question", example.get("problem"))
            answer = example.get("gold_answer", example.get("answer"))
            if question is None or answer is None:
                raise ValueError(
                    f"{self.path} record {index} must include question/problem and gold_answer/answer."
                )
...
def get_dataset_handler(dataset_name: str,name: str = None) -> DatasetHandler:
    if dataset_name == "math":
        ...
        return LocalJsonDatasetHandler(path=name)
    else:
