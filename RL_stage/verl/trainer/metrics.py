...
def compute_length_metrics(batch: DataProto) -> dict[str, Any]:
    ...
    visible_response_mask = batch.batch.get(
        "valid_response_mask", batch.batch["attention_mask"][:, -max_response_length:]
    )
    response_length = visible_response_mask.sum(-1).float()
    policy_response_length = batch.batch["response_mask"].sum(-1).float()
    ...
    return {
        ...
        "policy_response_length/mean": torch.mean(policy_response_length).detach().item(),
        "policy_response_length/max": torch.max(policy_response_length).detach().item(),
        "policy_response_length/min": torch.min(policy_response_length).detach().item(),
        "observation_response_length/mean": torch.mean(response_length - policy_response_length).detach().item(),
        # prompt length
...
def compute_timing_metrics(batch: DataProto, timing_raw: dict[str, float]) -> dict[str, Any]:
    num_response_tokens = torch.sum(batch.batch.get("valid_response_mask", batch.batch["response_mask"])).item()
